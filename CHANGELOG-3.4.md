...
Previous change logs can be found at [CHANGELOG-3.3](https://go.etcd.io/etcd/blob/master/CHANGELOG-3.3.md).

## v3.4.0 (TBD 2018-08)

See [code changes](https://go.etcd.io/etcd/compare/v3.3.0...v3.4.0) and [v3.4 upgrade guide](https://go.etcd.io/etcd/blob/master/Documentation/upgrades/upgrade_3_4.md) for any breaking changes. **Again, before running upgrades from any previous release, please make sure to read change logs below and [v3.4 upgrade guide](https://go.etcd.io/etcd/blob/master/Documentation/upgrades/upgrade_3_4.md).**

### Improved

- Rewrite [client balancer](TODO) with [new gRPC balancer interface](TODO).
- Add [jitter to watch progress notify](https://go.etcd.io/etcd/pull/9278) to prevent [spikes in `etcd_network_client_grpc_sent_bytes_total`](https://go.etcd.io/etcd/issues/9246).
- Improve [slow requests warning logging](https://go.etcd.io/etcd/pull/9288).
  - e.g. `etcdserver: read-only range request "key:\"\\000\" range_end:\"\\000\" " took too long [3.389041388s] to execute`
- Improve [TLS setup error logging](https://go.etcd.io/etcd/pull/9518) to help debug [TLS-enabled cluster configuring issues](https://go.etcd.io/etcd/issues/9400).
- Improve [long-running concurrent read transactions under light write workloads](https://go.etcd.io/etcd/pull/9296).
...
- Add [Raft Pre-Vote feature](https://go.etcd.io/etcd/pull/9352) to reduce [disruptive rejoining servers](https://go.etcd.io/etcd/issues/9333).
...
- Improve [lease expire/revoke operation performance](https://go.etcd.io/etcd/pull/9418), address [lease scalability issue](https://go.etcd.io/etcd/issues/9496).
- Make [Lease `Lookup` non-blocking with concurrent `Grant`/`Revoke`](https://go.etcd.io/etcd/pull/9229).
- Make etcd server return `raft.ErrProposalDropped` on internal Raft proposal drop in [v3 applier](https://go.etcd.io/etcd/pull/9549) and [v2 applier](https://go.etcd.io/etcd/pull/9558).
  - e.g. a node is removed from cluster, or [`raftpb.MsgProp` arrives at current leader while there is an ongoing leadership transfer](https://go.etcd.io/etcd/issues/8975).
- Add [`snapshot`](https://go.etcd.io/etcd/pull/9118) package for easier snapshot workflow (see [`godoc.org/github.com/etcd/clientv3/snapshot`](https://godoc.org/go.etcd.io/etcd/clientv3/snapshot) for more).
- Improve [functional tester](https://go.etcd.io/etcd/tree/master/functional) coverage: [proxy layer to run network fault tests in CI](https://go.etcd.io/etcd/pull/9081), [TLS is enabled both for server and client](https://go.etcd.io/etcd/pull/9534), [liveness mode](https://go.etcd.io/etcd/issues/9230), [shuffle test sequence](https://go.etcd.io/etcd/issues/9381), [membership reconfiguration failure cases](https://go.etcd.io/etcd/pull/9564), [disastrous quorum loss and snapshot recover from a seed member](https://go.etcd.io/etcd/pull/9565), [embedded etcd](https://go.etcd.io/etcd/pull/9572).
- Improve [index compaction blocking](https://go.etcd.io/etcd/pull/9511) by using a copy on write clone to avoid holding the lock for the traversal of the entire index.

### Breaking Changes

- **Remove `etcd --ca-file` flag**, instead [use `--trusted-ca-file`](https://go.etcd.io/etcd/pull/9470) (`--ca-file` has been deprecated since v2.1).
- **Remove `etcd --peer-ca-file` flag**, instead [use `--peer-trusted-ca-file`](https://go.etcd.io/etcd/pull/9470) (`--peer-ca-file` has been deprecated since v2.1).
- **Remove `pkg/transport.TLSInfo.CAFile` field**, instead [use `pkg/transport.TLSInfo.TrustedCAFile`](https://go.etcd.io/etcd/pull/9470) (`CAFile` has been deprecated since v2.1).
...
- Drop [ACIs from official release](https://go.etcd.io/etcd/pull/9059).
  - [AppC was officially suspended](https://github.com/appc/spec#-disclaimer-), as of late 2016.
  - [`acbuild`](https://github.com/containers/build#this-project-is-currently-unmaintained) is not maintained anymore.
  - `*.aci` files are not available from `v3.4` release.
- Exit on [empty hosts in advertise URLs](https://go.etcd.io/etcd/pull/8786).
...
- Exit on [shadowed environment variables](https://go.etcd.io/etcd/pull/9382).
...
- Change [`etcdserverpb.AuthRoleRevokePermissionRequest/key,range_end` fields type from `string` to `bytes`](https://go.etcd.io/etcd/pull/9433).
...
- Rename `embed.Config.SnapCount` field to [`embed.Config.SnapshotCount`](https://go.etcd.io/etcd/pull/9745), to be consistent with the flag name `etcd --snapshot-count`.
- Change [`embed.Config.CorsInfo` in `*cors.CORSInfo` type to `embed.Config.CORS` in `map[string]struct{}` type](https://go.etcd.io/etcd/pull/9490).
- Remove [`embed.Config.SetupLogging`](https://go.etcd.io/etcd/pull/9572).
  - Now logger is set up automatically based on [`embed.Config.Logger`, `embed.Config.LogOutputs`, `embed.Config.Debug` fields](https://go.etcd.io/etcd/pull/9572).
- Rename [`etcd --log-output` to `--log-outputs`](https://go.etcd.io/etcd/pull/9624) to support multiple log outputs.
  - **`etcd --log-output`** will be deprecated in v3.5.
- Rename [**`embed.Config.LogOutput`** to **`embed.Config.LogOutputs`**](https://go.etcd.io/etcd/pull/9624) to support multiple log outputs.
- Change [**`embed.Config.LogOutputs`** type from `string` to `[]string`](https://go.etcd.io/etcd/pull/9579) to support multiple log outputs.
...
- Change v3 `etcdctl snapshot` exit codes with [`snapshot` package](https://go.etcd.io/etcd/pull/9118/commits/df689f4280e1cce4b9d61300be13ca604d41670a).
  - Exit on error with exit code 1 (no more exit code 5 or 6 on `snapshot save/restore` commands).
- Migrate dependency management tool from `glide` to [`golang/dep`](https://go.etcd.io/etcd/pull/9155).
  - <= 3.3 puts `vendor` directory under `cmd/vendor` directory to [prevent conflicting transitive dependencies](https://go.etcd.io/etcd/issues/4913).
  - 3.4 moves `cmd/vendor` directory to `vendor` at repository root.
  - Remove recursive symlinks in `cmd` directory.
  - Now `go get/install/build` on `etcd` packages (e.g. `clientv3`, `tools/benchmark`) enforce builds with etcd `vendor` directory.
- Replace [gRPC gateway](https://github.com/grpc-ecosystem/grpc-gateway) endpoint `/v3beta` with [`/v3`](https://go.etcd.io/etcd/pull/9298).
  - Deprecated [`/v3alpha`](https://go.etcd.io/etcd/pull/9298).
  - To deprecate [`/v3beta`](https://go.etcd.io/etcd/issues/9189) in v3.5.
...
- Change [`wal` package function signatures](https://go.etcd.io/etcd/pull/9572) to support [structured logger and logging to file](https://go.etcd.io/etcd/issues/9438) in server-side.
...
- Remove [`pkg/cors` package](https://go.etcd.io/etcd/pull/9490).
...
- Move internal packages to `etcdserver`.
  - `"go.etcd.io/etcd/alarm"` to `"go.etcd.io/etcd/etcdserver/api/v3alarm"`.
  - `"go.etcd.io/etcd/compactor"` to `"go.etcd.io/etcd/etcdserver/api/v3compactor"`.
  - `"go.etcd.io/etcd/discovery"` to `"go.etcd.io/etcd/etcdserver/api/v2discovery"`.
  - `"go.etcd.io/etcd/etcdserver/auth"` to `"go.etcd.io/etcd/etcdserver/api/v2auth"`.
  - `"go.etcd.io/etcd/etcdserver/membership"` to `"go.etcd.io/etcd/etcdserver/api/membership"`.
  - `"go.etcd.io/etcd/etcdserver/stats"` to `"go.etcd.io/etcd/etcdserver/api/v2stats"`.
  - `"go.etcd.io/etcd/error"` to `"go.etcd.io/etcd/etcdserver/api/v2error"`.
  - `"go.etcd.io/etcd/rafthttp"` to `"go.etcd.io/etcd/etcdserver/api/rafthttp"`.
  - `"go.etcd.io/etcd/snap"` to `"go.etcd.io/etcd/etcdserver/api/snap"`.
  - `"go.etcd.io/etcd/store"` to `"go.etcd.io/etcd/etcdserver/api/v2store"`.
...
- Upgrade [`github.com/ugorji/go/codec`](https://github.com/ugorji/go/releases) to [**`v1.1.1`**](https://github.com/ugorji/go/releases/tag/v1.1.1), and [regenerate v2 `client`](https://go.etcd.io/etcd/pull/9494).
...
- Increase [`etcd_network_peer_round_trip_time_seconds`](https://go.etcd.io/etcd/pull/9762) Prometheus metric histogram upper-bound.
...
- Increase [`etcd_debugging_mvcc_index_compaction_pause_duration_milliseconds`](https://go.etcd.io/etcd/pull/9762) Prometheus metric histogram upper-bound.
...
- Add [`etcd_server_heartbeat_send_failures_total`](https://go.etcd.io/etcd/pull/9761) Prometheus metric.
- Add [`etcd_server_slow_apply_total`](https://go.etcd.io/etcd/pull/9761) Prometheus metric.
- Add [`etcd_snap_fsync_duration_seconds`](https://go.etcd.io/etcd/pull/9762) Prometheus metric.
- Add [`etcd_disk_backend_defrag_duration_seconds`](https://go.etcd.io/etcd/pull/9761) Prometheus metric.
- Add [`etcd_mvcc_hash_duration_seconds`](https://go.etcd.io/etcd/pull/9761) Prometheus metric.
- Add [`etcd_mvcc_hash_rev_duration_seconds`](https://go.etcd.io/etcd/pull/9761) Prometheus metric.
- Add [`etcd_debugging_mvcc_db_total_size_in_use_in_bytes`](https://go.etcd.io/etcd/pull/9256) Prometheus metric.
- Add [`etcd_network_active_peers`](https://go.etcd.io/etcd/pull/9762) Prometheus metric.
...
- Add [`etcd_network_disconnected_peers_total`](https://go.etcd.io/etcd/pull/9762) Prometheus metric.
...
- Add [`etcd_network_server_stream_failures_total`](https://go.etcd.io/etcd/pull/9760) Prometheus metric.
...
- Add [`etcd --host-whitelist`](https://go.etcd.io/etcd/pull/9372) flag, [`etcdserver.Config.HostWhitelist`](https://go.etcd.io/etcd/pull/9372), and [`embed.Config.HostWhitelist`](https://go.etcd.io/etcd/pull/9372), to prevent ["DNS Rebinding"](https://en.wikipedia.org/wiki/DNS_rebinding) attack.
...
- Support [`etcd --cors`](https://go.etcd.io/etcd/pull/9490) in v3 HTTP requests (gRPC gateway).
- Support [TLS cipher suite lists](TODO).
- Support [`ttl` field for `etcd` Authentication JWT token](https://go.etcd.io/etcd/pull/8302).
  - e.g. `etcd --auth-token jwt,pub-key=<pub key path>,priv-key=<priv key path>,sign-method=<sign method>,ttl=5m`.
- Allow empty token provider in [`etcdserver.ServerConfig.AuthToken`](https://go.etcd.io/etcd/pull/9369).
...
- Add [`--pre-vote`](https://go.etcd.io/etcd/pull/9352) flag to enable to run an additional Raft election phase.
  - For instance, a flaky(or rejoining) member may drop in and out, and start campaign. This member will end up with a higher term, and ignore all incoming messages with lower term. In this case, a new leader eventually need to get elected, thus disruptive to cluster availability. Raft implements Pre-Vote phase to prevent this kind of disruptions. If enabled, Raft runs an additional phase of election to check if pre-candidate can get enough votes to win an election.
...
- [`--enable-v2v3`](TODO) flag is now stable.
  ...
  - Added [more v2v3 integration tests](https://go.etcd.io/etcd/pull/9634).
  ...
  - Automatically [create parent directory if it does not exist](https://go.etcd.io/etcd/pull/9626) (fix [issue#9609](https://go.etcd.io/etcd/issues/9609)).
  - v4.0 will configure `--enable-v2=true --enable-v2v3=/aaa` to enable v2 API server that is backed by **v3 storage**.
- Add [`--discovery-srv-name`](https://go.etcd.io/etcd/pull/8690) flag to support custom DNS SRV name with discovery.
...
- Rename [`etcd --log-output` to `--log-outputs`](https://go.etcd.io/etcd/pull/9624) to support multiple log outputs.
  - **`etcd --log-output` will be deprecated in v3.5**.
- Add [`--logger`](https://go.etcd.io/etcd/pull/9572) flag to support [structured logger and multiple log outputs](https://go.etcd.io/etcd/issues/9438) in server-side.
  ...
  - `etcd --logger=zap --log-outputs=default` is not supported when `--logger=zap`.
    ...
    - However, even with PPID 1, it can fail to dial systemd journal (e.g. run embedded etcd with Docker container). Then, [every single log write will fail](https://go.etcd.io/etcd/pull/9729) and fall back to `os.Stderr`, which is inefficient.
  ...
  - `etcd --logger=zap --log-outputs=a.log,b.log,c.log,stdout` [writes server logs to multiple files `a.log`, `b.log` and `c.log` at the same time](https://go.etcd.io/etcd/pull/9579) and outputs to `os.Stderr`, in [JSON-encoded format](https://godoc.org/go.uber.org/zap#NewProductionEncoderConfig).
  - `etcd --logger=zap --log-outputs=/dev/null` will discard all server logs.
- Fix [`mvcc` "unsynced" watcher restore operation](https://go.etcd.io/etcd/pull/9281).
...
- Fix [revision-based compaction retention parsing](https://go.etcd.io/etcd/pull/9339).
  - Previously, `etcd --auto-compaction-mode revision --auto-compaction-retention 1` was [translated to revision retention 3600000000000](https://go.etcd.io/etcd/issues/9337).
...
- Allow [non-WAL files in `--wal-dir` directory](https://go.etcd.io/etcd/pull/9743).
  - Previously, existing files such as [`lost+found`](https://go.etcd.io/etcd/issues/7287) in WAL directory prevent etcd server boot.
  - Now, WAL directory that contains only `lost+found` or a file that's not suffixed with `.wal` is considered non-initialized.

### API

- Add [`snapshot`](https://go.etcd.io/etcd/pull/9118) package for snapshot restore/save operations (see [`godoc.org/github.com/etcd/clientv3/snapshot`](https://godoc.org/go.etcd.io/etcd/clientv3/snapshot) for more).
- Add [`watch_id` field to `etcdserverpb.WatchCreateRequest`](https://go.etcd.io/etcd/pull/9065) to allow user-provided watch ID to `mvcc`.
  - Corresponding `watch_id` is returned via `etcdserverpb.WatchResponse`, if any.
- Add [`fragment` field to `etcdserverpb.WatchCreateRequest`](https://go.etcd.io/etcd/pull/9291) to request etcd server to [split watch events](https://go.etcd.io/etcd/issues/9294) when the total size of events exceeds `etcd --max-request-bytes` flag value plus gRPC-overhead 512 bytes.
...
- Add [`raftAppliedIndex` field to `etcdserverpb.StatusResponse`](https://go.etcd.io/etcd/pull/9176) for current Raft applied index.
- Add [`errors` field to `etcdserverpb.StatusResponse`](https://go.etcd.io/etcd/pull/9206) for server-side error.
  - e.g. `"etcdserver: no leader", "NOSPACE", "CORRUPT"`
- Add [`dbSizeInUse` field to `etcdserverpb.StatusResponse`](https://go.etcd.io/etcd/pull/9256) for actual DB size after compaction.
...
- Define [`embed.CompactorModePeriodic`](https://godoc.org/go.etcd.io/etcd/embed#pkg-variables) for `compactor.ModePeriodic`.
- Define [`embed.CompactorModeRevision`](https://godoc.org/go.etcd.io/etcd/embed#pkg-variables) for `compactor.ModeRevision`.
- Change [`embed.Config.CorsInfo` in `*cors.CORSInfo` type to `embed.Config.CORS` in `map[string]struct{}` type](https://go.etcd.io/etcd/pull/9490).
- Remove [`embed.Config.SetupLogging`](https://go.etcd.io/etcd/pull/9572).
  - Now logger is set up automatically based on [`embed.Config.Logger`, `embed.Config.LogOutputs`, `embed.Config.Debug` fields](https://go.etcd.io/etcd/pull/9572).
- Add [`embed.Config.Logger`](https://go.etcd.io/etcd/pull/9518) to support [structured logger `zap`](https://github.com/uber-go/zap) in server-side.
- Rename `embed.Config.SnapCount` field to [`embed.Config.SnapshotCount`](https://go.etcd.io/etcd/pull/9745), to be consistent with the flag name `etcd --snapshot-count`.
- Rename [**`embed.Config.LogOutput`** to **`embed.Config.LogOutputs`**](https://go.etcd.io/etcd/pull/9624) to support multiple log outputs.
- Change [**`embed.Config.LogOutputs`** type from `string` to `[]string`](https://go.etcd.io/etcd/pull/9579) to support multiple log outputs.
...
- Add [`CLUSTER_DEBUG` to enable test cluster logging](https://go.etcd.io/etcd/pull/9678).
  - Deprecated `capnslog` in integration tests.

### client v3

- Add [`WithFragment` `OpOption`](https://go.etcd.io/etcd/pull/9291) to support [watch events fragmentation](https://go.etcd.io/etcd/issues/9294) when the total size of events exceeds `etcd --max-request-bytes` flag value plus gRPC-overhead 512 bytes.
...
- Add [`check datascale`](https://go.etcd.io/etcd/pull/9185) command.
- Add [`check datascale --auto-compact, --auto-defrag`](https://go.etcd.io/etcd/pull/9351) flags.
- Add [`check perf --auto-compact, --auto-defrag`](https://go.etcd.io/etcd/pull/9330) flags.
- Add [`defrag --cluster`](https://go.etcd.io/etcd/pull/9390) flag.
- Add ["raft applied index" field to `endpoint status`](https://go.etcd.io/etcd/pull/9176).
- Add ["errors" field to `endpoint status`](https://go.etcd.io/etcd/pull/9206).
- Add [`endpoint health --write-out` support](https://go.etcd.io/etcd/pull/9540).
  - Previously, [`endpoint health --write-out json` did not work](https://go.etcd.io/etcd/issues/9532).
...
- Replace [gRPC gateway](https://github.com/grpc-ecosystem/grpc-gateway) endpoint `/v3beta` with [`/v3`](https://go.etcd.io/etcd/pull/9298).
  ...
  - To deprecate [`/v3beta`](https://go.etcd.io/etcd/issues/9189) in v3.5.
  - In v3.4, `curl -L http://localhost:2379/v3beta/kv/put -X POST -d '{"key": "Zm9v", "value": "YmFy"}'` still works as a fallback to `curl -L http://localhost:2379/v3/kv/put -X POST -d '{"key": "Zm9v", "value": "YmFy"}'`, but `curl -L http://localhost:2379/v3beta/kv/put -X POST -d '{"key": "Zm9v", "value": "YmFy"}'` won't work in v3.5. Use `curl -L http://localhost:2379/v3/kv/put -X POST -d '{"key": "Zm9v", "value": "YmFy"}'` instead.
- Add API endpoints [`/{v3beta,v3}/lease/leases, /{v3beta,v3}/lease/revoke, /{v3beta,v3}/lease/timetolive`](https://go.etcd.io/etcd/pull/9450).
  - To deprecate [`/{v3beta,v3}/kv/lease/leases, /{v3beta,v3}/kv/lease/revoke, /{v3beta,v3}/kv/lease/timetolive`](https://go.etcd.io/etcd/issues/9430) in v3.5.
- Support [`etcd --cors`](https://go.etcd.io/etcd/pull/9490) in v3 HTTP requests (gRPC gateway).

### Package `raft`

- Fix [deadlock during PreVote migration process](https://go.etcd.io/etcd/pull/8525).
- Add [`raft.ErrProposalDropped`](https://go.etcd.io/etcd/pull/9067).
  - Now [`(r *raft) Step` returns `raft.ErrProposalDropped`](https://go.etcd.io/etcd/pull/9137) if a proposal has been ignored.
  - e.g. a node is removed from cluster, or [`raftpb.MsgProp` arrives at current leader while there is an ongoing leadership transfer](https://go.etcd.io/etcd/issues/8975).
- Improve [Raft `becomeLeader` and `stepLeader`](https://go.etcd.io/etcd/pull/9073) by keeping track of latest `pb.EntryConfChange` index.
  - Previously record `pendingConf` boolean field scanning the entire tail of the log, which can delay hearbeat send.
- Fix [missing learner nodes on `(n *node) ApplyConfChange`](https://go.etcd.io/etcd/pull/9116).

### Tooling

- Add [`etcd-dump-logs --entry-type`](https://go.etcd.io/etcd/pull/9628) flag to support WAL log filtering by entry type.
